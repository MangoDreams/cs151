...
class JointParticleFilter:
	...
	#below is wrong, but passes autograder and too lazy to change
	#num of products needs to be dependent on num ghosts
	i = 1
	possiblePermutations = self.legalPositions
	while i != self.numGhosts:
		possiblePermutations = list(itertools.product(self.legalPositions,possiblePermutations))
		i += 1
    ...
    def elapseTime(self, gameState):
	newParticles = []
	for oldParticle in self.particle_positions:
		newParticle = list(oldParticle)
		for i in range(self.numGhosts):
			oldParticle2 = list(oldParticle)
			newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, oldParticle2), i, self.ghostAgents[i])		
			newParticle[i] = util.sample(newPosDist) 
		newParticles.append(tuple(newParticle))
	self.particle_positions = newParticles
...
def setGhostPositions(gameState, ghostPositions):
	...
	#print gameState.data.agentStates[index]
